...
def verify_payment(request):
    ...
    if not all([razorpay_payment_id, razorpay_order_id, razorpay_signature, amount]):
        return Response(
            {"detail": "All payment details and amount are required"},
    ...
    try:
        ...
        wallet.balance += max(
            decimal.Decimal(str(amount_decimal)), decimal.Decimal("0.00")
        )
...
def get_remittances(request):
    ...
    for order in delivered_cod:
        if (
            not CODRemittance.objects.filter(order=order).exists()
            and order.cod_amount > 0
        ):
    ...
    qs = (
        CODRemittance.objects.filter(user=request.user)
...
def remittance_summary(request):
    ...
    total_remitted = (
        qs.filter(status="TRANSFERRED").aggregate(total=Sum("cod_amount"))["total"] or 0
    )
    total_pending = (
        qs.filter(status="PENDING").aggregate(total=Sum("cod_amount"))["total"] or 0
    )
    ...
    active_cod_qs = Order.objects.filter(user=request.user, order_type="COD").exclude(
...
def transfer_remittance(request, pk):
    ...
    try:
        ...
        return Response(
            {"detail": "Remittance not found."}, status=status.HTTP_404_NOT_FOUND
        )

    if remittance.status == "TRANSFERRED":
        return Response(
            {"detail": "Already transferred."}, status=status.HTTP_400_BAD_REQUEST
        )
...
def transfer_all_remittances(request):
    ...
    if not pending.exists():
        return Response(
            {"detail": "No pending remittances to transfer."},
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    if total_amount <= 0:
        return Response(
            {"detail": "No amount to transfer."}, status=status.HTTP_400_BAD_REQUEST
        )
