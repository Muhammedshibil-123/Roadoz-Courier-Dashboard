...
const ProcessingOrders = () => {
  ...
  const fetchOrders = async () => {
    try { const res = await api.get('/api/orders/?status=PROCESSING', { skipLoading: true }); setOrders(res.data); }
  ...
  const handleBulkDelete = async () => {
    if (selectedOrders.length === 0) { alert('Select at least one order.'); return; }
    if (!window.confirm(`Delete ${selectedOrders.length} order(s)? This cannot be undone.`)) return;
    setDeleting(true);
    try {
      const res = await api.post('/api/orders/bulk-delete/', { ids: selectedOrders });
      alert(res.data.detail);
      setOrders(prev => prev.filter(o => !selectedOrders.includes(o.id)));
      setSelectedOrders([]);
    } catch (err) { alert(err.response?.data?.detail || 'Failed to delete orders.'); }
    setDeleting(false);
  };
  ...
  return (
    <div className="flex-1 p-6 space-y-5">
      <OrderFilterBar
        ...
        filters={filters}
        actionButtons={<>
          <button onClick={() => navigate('/settings/pickup-address')}
          ...
          <button onClick={() => exportOrdersCsv(filteredOrders, 'processing_orders')}
          ...
          <button onClick={handleBulkDelete} disabled={deleting || selectedOrders.length === 0}
            ...
            <FaTrash className="text-[10px]" /> {deleting ? 'Deleting...' : `Delete${selectedOrders.length > 0 ? ` (${selectedOrders.length})` : ''}`}
          </button>
        </>}
      />
          ...
          <table className="w-full min-w-[900px]">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan="8" className="p-8 text-center text-sm text-[var(--color-text-secondary)]">
                  {filters.hasActiveFilters ? 'No orders match your filters' : 'No processing orders'}
                  {filters.hasActiveFilters && <button onClick={filters.clearFilters} className="text-xs text-[#d4af26] hover:underline ml-2">Clear filters</button>}
                </td></tr>
                ...
                <tr key={order.id} className="border-b border-[var(--color-border)] hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                  ...
                  <td className="p-3"><span className="text-sm font-bold text-[#d4af26]">{order.tracking_id}</span></td>
                  <td className="p-3">
                    <div className="flex items-center gap-1 text-xs text-[var(--color-text-secondary)]"><MdLocationOn className="text-sm flex-shrink-0" /><span>{order.destination_pincode}</span></div>
                  ...
                  <td className="p-3">
                    ...
                    <span className="text-sm text-[var(--color-text-primary)] block">₹{order.order_type === 'COD' ? order.cod_amount : order.product_amount}</span>
                  </td>
                  <td className="p-3"><span className="text-xs font-medium text-[var(--color-text-primary)]">{order.weight} kg</span></td>
                  <td className="p-3"><span className="text-[11px] text-[var(--color-text-secondary)]">{new Date(order.created_at).toLocaleString('en-IN', { dateStyle: 'medium', timeStyle: 'short' })}</span></td>
                  <td className="p-3 text-center">
                    <button onClick={() => handleAdvance(order.id)} disabled={advancing === order.id}
                      className="w-8 h-8 bg-green-500 hover:bg-green-600 text-white rounded-md flex items-center justify-center mx-auto transition-colors disabled:opacity-50" title="Tick to Manifest">
                      {advancing === order.id ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <FaCheck className="text-sm" />}
                    </button>
                  </td>
                </tr>
              ))}
