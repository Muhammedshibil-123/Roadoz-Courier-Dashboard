...
const Reports = () => {
    ...
    try {
      ...
      if (isPdf) {
        ...
        autoTable(doc, {
          ...
          body: tableBody,
          theme: "grid",
          headStyles: { fillColor: [212, 175, 38], textColor: 255 },
        ...
        link.setAttribute(
          "download",
          `roadoz_orders_${new Date().toISOString().split("T")[0]}.csv`,
        );
  ...
  const inputClass =
    "bg-transparent border border-[var(--color-border)] rounded-md px-3 py-2 text-sm text-[var(--color-text-primary)] focus:outline-none focus:border-[#d4af26] transition-colors";
      ...
      <div className="bg-[var(--color-bg-surface)] rounded-lg p-5 border border-[var(--color-border)] flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs mt-1 text-[var(--color-text-secondary)]">
            Export your shipping and order data. Customize the date range and
            exactly which columns you need.
          </p>
        </div>
      </div>
        ...
        <div className="lg:col-span-2">
          <div className="bg-[var(--color-bg-surface)] rounded-lg p-6 border border-[var(--color-border)] h-full">
            ...
            <p className="text-xs text-[var(--color-text-secondary)] mb-6">
              Choose the specific data points you want included in your export.
              Unchecked columns will be omitted. Depending on the amount of
              columns, PDF will auto-scale to fit.
            </p>
              ...
              {AVAILABLE_FIELDS.map((field) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`text-sm font-medium ${isSelected ? "text-[var(--color-text-primary)]" : "text-[var(--color-text-secondary)]"}`}
                    >
                      {field}
                    </span>
                  </div>
