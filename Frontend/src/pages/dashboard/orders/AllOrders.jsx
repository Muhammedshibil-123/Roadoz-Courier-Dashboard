...
import { OrderFilterBar, OrderPagination, useOrderFilters, exportOrdersCsv, STATUS_MAP } from '../../../components/OrderFilterBar';
...
const AllOrders = () => {
  ...
  useEffect(() => {
    (async () => {
      try { const res = await api.get('/api/orders/', { skipLoading: true }); setOrders(res.data); }
  ...
  return (
    <div className="flex-1 p-6 space-y-5">
      <OrderFilterBar title="All Orders" currentPath="/orders/all" filters={filters}
        actionButtons={
          <button onClick={() => exportOrdersCsv(filteredOrders, 'all_orders')} disabled={filteredOrders.length === 0}
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan="8" className="p-8 text-center text-sm text-[var(--color-text-secondary)]">
                  {filters.hasActiveFilters ? 'No orders match your filters' : 'No orders found'}
                </td></tr>
              ) : paginatedOrders.map(o => {
                const s = STATUS_MAP[o.status] || { label: o.status || 'Unknown', badge: 'bg-gray-500' };
                return (
                  <tr key={o.id} className="border-b border-[var(--color-border)] hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                    ...
                    <td className="p-3">
                      ...
                      <span className="text-sm text-[var(--color-text-primary)] block">₹{o.order_type === 'COD' ? o.cod_amount : o.product_amount}</span>
                    </td>
                    ...
                    <td className="p-3 text-center">
                      <span className={`text-[9px] font-bold text-white px-2.5 py-1 rounded-full ${s.badge}`}>{s.label}</span>
                    </td>
                  </tr>
                );
              })}
