...
const InTransit = () => {
  ...
  useEffect(() => {
    (async () => {
      try { const res = await api.get('/api/orders/?status=IN_TRANSIT', { skipLoading: true }); setOrders(res.data); }
  ...
  return (
    <div className="flex-1 p-6 space-y-5">
      <OrderFilterBar title="In Transit Orders" currentPath="/orders/in-transit" filters={filters}
        actionButtons={
          <button onClick={() => exportOrdersCsv(filteredOrders, 'in_transit_orders')} disabled={filteredOrders.length === 0}
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan="7" className="p-8 text-center text-sm text-[var(--color-text-secondary)]">
                  {filters.hasActiveFilters ? 'No orders match your filters' : 'No in-transit orders'}
                </td></tr>
