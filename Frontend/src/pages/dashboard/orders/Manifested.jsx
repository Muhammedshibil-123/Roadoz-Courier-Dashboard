...
const Manifested = () => {
  ...
  const fetchOrders = async () => {
    try { const res = await api.get('/api/orders/?status=MANIFESTED', { skipLoading: true }); setOrders(res.data); }
  ...
  return (
    <div className="flex-1 p-6 space-y-5">
      <OrderFilterBar title="Manifested Orders" currentPath="/orders/manifested" filters={filters}
        actionButtons={<>
          <button onClick={() => exportOrdersCsv(filteredOrders, 'manifested_orders')} disabled={filteredOrders.length === 0}
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan="8" className="p-8 text-center text-sm text-[var(--color-text-secondary)]">
                  {filters.hasActiveFilters ? 'No orders match your filters' : 'No manifested orders'}
                </td></tr>
                ...
                <tr key={o.id} className="border-b border-[var(--color-border)] hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                  ...
                  <td className="p-3">
                    <div className="flex items-center gap-1 text-xs text-[var(--color-text-secondary)]"><MdLocationOn className="text-sm flex-shrink-0" /><span>{o.destination_pincode}</span></div>
                    <span className="text-[10px] text-[var(--color-text-secondary)]">{o.destination_address?.substring(0, 30)}</span>
                  </td>
                  <td className="p-3">
                  ...
                  <td className="p-3 text-center">
                    <button onClick={() => handleAdvance(o.id)} disabled={advancing === o.id}
                      className="w-8 h-8 bg-blue-500 hover:bg-blue-600 text-white rounded-md flex items-center justify-center mx-auto transition-colors disabled:opacity-50" title="Tick to send for Pickup">
