...
const Manifested = () => {
  ...
  const fetchOrders = async () => {
    try {
      const res = await api.get("/api/orders/?status=MANIFESTED", {
        skipLoading: true,
      });
      setOrders(res.data);
    } catch {
  ...
  return (
    <div className="flex-1 p-6 space-y-5">
      <OrderFilterBar
        ...
        currentPath="/orders/manifested"
        filters={filters}
        actionButtons={
          <>
            <button
              onClick={() =>
                exportOrdersCsv(filteredOrders, "manifested_orders")
              }
                ...
                paginatedOrders.map((o) => (
                  ...
                  >
                    ...
                    <td className="p-3">
                      ...
                      <span className="text-[9px] font-bold text-white px-2 py-0.5 rounded bg-[#d4af26] ml-2">
                        MANIFESTED
                      </span>
                    </td>
                    <td className="p-3">
