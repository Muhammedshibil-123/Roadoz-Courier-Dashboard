...
const NDR = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await api.get("/api/orders/?status=NDR", {
          skipLoading: true,
        });
        setOrders(res.data);
      } catch {
  ...
  return (
    <div className="flex-1 p-6 space-y-5">
      <OrderFilterBar
        title="NDR Orders"
        currentPath="/orders/ndr"
        filters={filters}
        actionButtons={
          <button
            onClick={() => exportOrdersCsv(filteredOrders, "ndr_orders")}
                ...
                paginatedOrders.map((o) => (
                  ...
                  >
                    ...
                    <td className="p-3">
                      ...
                      <span className="text-[9px] font-bold text-white px-2 py-0.5 rounded bg-orange-600 ml-2">
                        NDR
                      </span>
                    </td>
