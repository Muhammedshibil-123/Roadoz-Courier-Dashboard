...
const NDR = () => {
  ...
  useEffect(() => {
    (async () => {
      try { const res = await api.get('/api/orders/?status=NDR', { skipLoading: true }); setOrders(res.data); }
  ...
  return (
    <div className="flex-1 p-6 space-y-5">
      <OrderFilterBar title="NDR Orders" currentPath="/orders/ndr" filters={filters}
        actionButtons={
          <button onClick={() => exportOrdersCsv(filteredOrders, 'ndr_orders')} disabled={filteredOrders.length === 0}
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan="7" className="p-8 text-center text-sm text-[var(--color-text-secondary)]">
                  {filters.hasActiveFilters ? 'No orders match your filters' : 'No NDR orders'}
                </td></tr>
                ...
                <tr key={o.id} className="border-b border-[var(--color-border)] hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                  ...
                  <td className="p-3">
                    ...
                    <span className="text-[9px] font-bold text-white px-2 py-0.5 rounded bg-orange-600 ml-2">NDR</span>
                  </td>
