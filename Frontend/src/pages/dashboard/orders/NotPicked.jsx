...
const NotPicked = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await api.get("/api/orders/?status=NOT_PICKED", {
          skipLoading: true,
        });
        setOrders(res.data);
      } catch {
  ...
  return (
    <div className="flex-1 p-6 space-y-5">
      <OrderFilterBar
        title="Not Picked Orders"
        currentPath="/orders/not-picked"
        filters={filters}
        actionButtons={
          <button
            onClick={() => exportOrdersCsv(filteredOrders, "not_picked_orders")}
                ...
                paginatedOrders.map((o) => (
                  ...
                  >
                    ...
                    <td className="p-3">
                      ...
                      <span className="text-[9px] font-bold text-white px-2 py-0.5 rounded bg-red-500 ml-2">
                        NOT PICKED
                      </span>
                    </td>
