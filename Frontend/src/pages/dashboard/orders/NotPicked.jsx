...
const NotPicked = () => {
  ...
  const [selectedStatus, setSelectedStatus] = useState('Not Picked');
  ...
  useEffect(() => {
    const f = async () => { try { const r = await api.get('/api/orders/?status=NOT_PICKED', { skipLoading: true }); setOrders(r.data); } catch { setOrders([]); } finally { setLoading(false); } }; f();
  }, []);

  return (
    <div className="flex-1 p-6 space-y-5">
      <div className="bg-[var(--color-bg-surface)] rounded-lg p-5 border border-[var(--color-border)]">
        <h1 className="text-xl font-semibold text-[var(--color-text-primary)]">Not Picked Orders</h1>
        <p className="text-xs mt-0.5"><span className="text-[#d4af26]">Dashboard</span><span className="text-[var(--color-text-secondary)]"> &gt;&gt; Not Picked Orders</span></p>
      </div>

      <div className="bg-[var(--color-bg-surface)] rounded-lg p-5 border border-[var(--color-border)] space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h2 className="text-lg font-semibold text-[var(--color-text-primary)]">Not Picked Orders</h2>
      ...
      <div className="border-b border-[var(--color-border)] overflow-x-auto"><div className="flex gap-0 min-w-max">{tabs.map((tab) => (<button key={tab.label} onClick={() => navigate(tab.path)} className={`px-4 py-2.5 text-xs font-medium whitespace-nowrap transition-colors border-b-2 ${tab.path === '/orders/not-picked' ? 'text-[var(--color-text-primary)] border-[#d4af26]' : 'text-[var(--color-text-secondary)] border-transparent hover:text-[var(--color-text-primary)]'}`}>{tab.label}({tab.count})</button>))}</div></div>
          ...
          <table className="w-full min-w-[900px]">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan="8" className="p-8 text-center text-sm text-[var(--color-text-secondary)]">No not-picked orders</td></tr>
              ) : orders.map((o) => (
                <tr key={o.id} className="border-b border-[var(--color-border)] hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                  ...
                  <td className="p-3"><span className="text-sm font-semibold text-[var(--color-text-primary)] block">{o.customer_name}</span><span className="text-xs text-[var(--color-text-secondary)]">{o.customer_phone}</span><span className="text-[9px] font-bold text-white px-2 py-0.5 rounded bg-red-500 ml-2">NOT PICKED</span></td>
