...
const OutOfDelivery = () => {
  ...
  const [selectedStatus, setSelectedStatus] = useState('Out of Delivery');
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const res = await api.get('/api/orders/?status=OUT_FOR_DELIVERY', { skipLoading: true });
        setOrders(res.data);
      } catch { setOrders([]); }
  ...
  return (
    <div className="flex-1 p-6 space-y-5">
      <div className="bg-[var(--color-bg-surface)] rounded-lg p-5 border border-[var(--color-border)]">
        <h1 className="text-xl font-semibold text-[var(--color-text-primary)]">Out of Delivery Orders</h1>
        <p className="text-xs mt-0.5"><span className="text-[#d4af26]">Dashboard</span><span className="text-[var(--color-text-secondary)]"> &gt;&gt; </span><span className="text-[var(--color-text-secondary)]">Out of Delivery Orders</span></p>
      </div>
      <div className="bg-[var(--color-bg-surface)] rounded-lg p-5 border border-[var(--color-border)] space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h2 className="text-lg font-semibold text-[var(--color-text-primary)]">Out of Delivery Orders</h2>
        ...
        <div className="flex flex-wrap items-start gap-4">
          <div>
            ...
            <div className="bg-white dark:bg-[#1a2332] border border-[var(--color-border)] rounded-md px-3 py-2 space-y-1.5">
              {['Out of Delivery', 'All Orders'].map((opt) => (
      ...
      <div className="border-b border-[var(--color-border)] overflow-x-auto"><div className="flex gap-0 min-w-max">{tabs.map((tab) => (<button key={tab.label} onClick={() => navigate(tab.path)} className={`px-4 py-2.5 text-xs font-medium whitespace-nowrap transition-colors border-b-2 ${tab.path === '/orders/out-of-delivery' ? 'text-[var(--color-text-primary)] border-[#d4af26]' : 'text-[var(--color-text-secondary)] border-transparent hover:text-[var(--color-text-primary)]'}`}>{tab.label}({tab.count})</button>))}</div></div>
          ...
          <table className="w-full min-w-[900px]">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan="8" className="p-8 text-center text-sm text-[var(--color-text-secondary)]">No out for delivery orders</td></tr>
              ) : orders.map((o) => (
                <tr key={o.id} className="border-b border-[var(--color-border)] hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                  ...
                  <td className="p-3"><span className="text-sm text-[var(--color-text-primary)]">{o.tracking_id}</span><span className="text-[9px] font-bold text-white px-2 py-0.5 rounded bg-blue-600 ml-2">OUT FOR DELIVERY</span></td>
                  <td className="p-3 text-xs text-[var(--color-text-secondary)]">{o.destination_pincode}</td>
                  <td className="p-3"><span className="text-sm text-[var(--color-text-primary)]">₹{o.cod_amount}</span><span className={`text-[10px] font-bold ml-1 ${o.order_type === 'COD' ? 'text-green-400' : 'text-[#d4af26]'}`}>{o.order_type}</span></td>
                  ...
                  <td className="p-3"><div className="flex items-center gap-1.5"><button className="p-1.5 bg-[#d4af26] text-white rounded hover:bg-[#c39f19]"><FaDownload className="text-xs" /></button><button className="p-1.5 text-[var(--color-text-secondary)] hover:text-[#d4af26]"><FaEllipsisV className="text-xs" /></button></div></td>
                </tr>
              ))}
            </tbody>
          </table>
