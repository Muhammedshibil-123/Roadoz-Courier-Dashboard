...
const OutOfDelivery = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await api.get("/api/orders/?status=OUT_FOR_DELIVERY", {
          skipLoading: true,
        });
        setOrders(res.data);
      } catch {
  ...
  return (
    <div className="flex-1 p-6 space-y-5">
      <OrderFilterBar
        ...
        currentPath="/orders/out-of-delivery"
        filters={filters}
        actionButtons={
          <button
            onClick={() =>
              exportOrdersCsv(filteredOrders, "out_for_delivery_orders")
            }
                ...
                paginatedOrders.map((o) => (
                  ...
                  >
                    ...
                    <td className="p-3">
                      ...
                      <span className="text-[9px] font-bold text-white px-2 py-0.5 rounded bg-blue-600 ml-2">
                        OUT FOR DELIVERY
                      </span>
                    </td>
