...
const Pending = () => {
  ...
  const fetchOrders = async () => {
    try {
      const res = await api.get("/api/orders/?status=PICKUP_PENDING", {
        skipLoading: true,
      });
      setOrders(res.data);
    } catch {
  ...
  const getTimeInfo = (statusChangedAt) => {
    ...
    if (remaining <= 0)
      return { text: "Expired — moving to Not Picked", color: "text-red-400" };
    if (remaining < 6)
      return {
        text: `${remaining.toFixed(1)}h remaining`,
  ...
  return (
    <div className="flex-1 p-6 space-y-5">
      <OrderFilterBar
        title="Pending Orders (Pickup Waiting)"
        currentPath="/orders/pending"
        filters={filters}
        actionButtons={
          <button
            onClick={() => exportOrdersCsv(filteredOrders, "pending_orders")}
      ...
      >
        <div className="bg-[#d4af26]/10 border border-[#d4af26]/30 rounded-lg px-5 py-3">
          <p className="text-sm text-[#d4af26]">
            ...
            Orders waiting for courier pickup. If not picked within{" "}
            <strong>24 hours</strong>, they auto-move to{" "}
            <strong>Not Picked</strong>. Tick ✓ to confirm pickup →{" "}
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-3">
                        ...
                        <span className="text-[9px] font-bold text-white px-2 py-0.5 rounded bg-orange-500 ml-2">
                          PICKUP PENDING
                        </span>
                      </td>
                      <td className="p-3">
                      ...
                      <td className="p-3">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span
                            className={`text-xs font-semibold ${timeInfo.color}`}
                          >
                            {timeInfo.text}
                          </span>
                        </div>
