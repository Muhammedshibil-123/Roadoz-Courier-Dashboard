...
const RTODelivery = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await api.get("/api/orders/?status=RTO_DELIVERED", {
          skipLoading: true,
        });
        setOrders(res.data);
      } catch {
  ...
  return (
    <div className="flex-1 p-6 space-y-5">
      <OrderFilterBar
        title="RTO Delivered Orders"
        currentPath="/orders/rto-delivery"
        filters={filters}
        actionButtons={
          <button
            onClick={() =>
              exportOrdersCsv(filteredOrders, "rto_delivered_orders")
            }
                ...
                paginatedOrders.map((o) => (
                  ...
                  >
                    ...
                    <td className="p-3">
                      ...
                      <span className="text-[9px] font-bold text-white px-2 py-0.5 rounded bg-purple-700 ml-2">
                        RTO DELIVERED
                      </span>
                    </td>
