...
const RTODelivery = () => {
  ...
  useEffect(() => {
    (async () => {
      try { const res = await api.get('/api/orders/?status=RTO_DELIVERED', { skipLoading: true }); setOrders(res.data); }
  ...
  return (
    <div className="flex-1 p-6 space-y-5">
      <OrderFilterBar title="RTO Delivered Orders" currentPath="/orders/rto-delivery" filters={filters}
        actionButtons={
          <button onClick={() => exportOrdersCsv(filteredOrders, 'rto_delivered_orders')} disabled={filteredOrders.length === 0}
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan="7" className="p-8 text-center text-sm text-[var(--color-text-secondary)]">
                  {filters.hasActiveFilters ? 'No orders match your filters' : 'No RTO delivered orders'}
                </td></tr>
                ...
                <tr key={o.id} className="border-b border-[var(--color-border)] hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                  ...
                  <td className="p-3">
                    ...
                    <span className="text-[9px] font-bold text-white px-2 py-0.5 rounded bg-purple-700 ml-2">RTO DELIVERED</span>
                  </td>
