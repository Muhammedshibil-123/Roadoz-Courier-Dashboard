...
const RTOInTransit = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await api.get("/api/orders/?status=RTO_IN_TRANSIT", {
          skipLoading: true,
        });
        setOrders(res.data);
      } catch {
  ...
  return (
    <div className="flex-1 p-6 space-y-5">
      <OrderFilterBar
        ...
        currentPath="/orders/rto-in-transit"
        filters={filters}
        actionButtons={
          <button
            onClick={() =>
              exportOrdersCsv(filteredOrders, "rto_in_transit_orders")
            }
                ...
                paginatedOrders.map((o) => (
                  ...
                  >
                    ...
                    <td className="p-3">
                      ...
                      <span className="text-[9px] font-bold text-white px-2 py-0.5 rounded bg-purple-500 ml-2">
                        RTO IN TRANSIT
                      </span>
                    </td>
