...
const ChangePassword = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (errorData?.error) {
        ...
        setError(
          Array.isArray(errorData.old_password)
  ...
  const inputClass =
    "bg-transparent border border-[var(--color-border)] rounded-md px-4 py-2.5 text-sm text-[var(--color-text-primary)] placeholder-[var(--color-text-secondary)] focus:outline-none focus:border-[#d4af26] transition-colors w-full pr-10";
          ...
          {[
            {
              ...
              met: form.new_password.length >= 6,
            },
            {
              text: "Contains uppercase letter",
              met: /[A-Z]/.test(form.new_password),
            },
            { text: "Contains a number", met: /[0-9]/.test(form.new_password) },
            {
              text: "Contains special character",
              met: /[^A-Za-z0-9]/.test(form.new_password),
            },
          ].map((req) => (
            <p
              key={req.text}
              className={`text-xs flex items-center gap-1.5 ${req.met ? "text-green-400" : "text-[var(--color-text-secondary)]"}`}
            >
              <span
                className={`w-1 h-1 rounded-full ${req.met ? "bg-green-400" : "bg-[var(--color-text-secondary)]"}`}
              />
              {req.text}
            </p>
          ))}
