...
import { FaIdCard, FaBuilding, FaUniversity, FaCheckCircle, FaClock, FaTimesCircle, FaExclamationTriangle } from 'react-icons/fa';
...
const KYC = () => {
  ...
  useEffect(() => {
    const fetchKYC = async () => {
      try {
        const res = await api.get('/api/auth/settings/kyc/', { skipLoading: true });
        const data = res.data;
        setForm({
  ...
  const handleSubmit = async () => {
    ...
    if (!form.full_name || !form.pan_number || !form.aadhar_number) {
      setError('Full Name, PAN Card Number, and Aadhar Card Number are required.');
      return;
    }
    setError('');
    setLoading(true);
  ...
  return (
    <div className="flex-1 p-6 space-y-5">
      ...
      <div className="bg-[var(--color-bg-surface)] rounded-lg p-5 border border-[var(--color-border)]">
        <h1 className="text-xl font-semibold text-[var(--color-text-primary)]">KYC Verification</h1>
        <p className="text-xs mt-0.5">
          <span className="text-[#d4af26]">Dashboard</span>
          <span className="text-[var(--color-text-secondary)]"> &gt; </span>
          <span className="text-[var(--color-text-secondary)]">KYC Verification</span>
        </p>
      </div>
      ...
      <div className="bg-[var(--color-bg-surface)] rounded-lg p-6 border border-[var(--color-border)] space-y-6">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-[var(--color-text-primary)]">KYC Details</h2>
          {getStatusBadge()}
        </div>
        ...
        {kycStatus === 'pending' && (
          <div className="bg-[#d4af26]/10 border border-[#d4af26]/30 rounded-lg px-5 py-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-[#d4af26]">KYC Under Review</p>
                <p className="text-xs text-[var(--color-text-secondary)] mt-0.5">
                  Your KYC documents have been submitted successfully. Our team will review your details and get back to you within 2-3 business days.
                </p>
              </div>
            </div>
          </div>
        ...
        {kycStatus === 'verified' && (
          <div className="bg-green-500/10 border border-green-500/30 rounded-lg px-5 py-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-green-400">KYC Verified</p>
                <p className="text-xs text-[var(--color-text-secondary)] mt-0.5">
                  Your KYC has been verified successfully. You now have full access to all features.
                </p>
              </div>
            </div>
          </div>
        ...
        {kycStatus === 'rejected' && (
          <div className="bg-red-500/10 border border-red-500/30 rounded-lg px-5 py-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-red-400">KYC Rejected</p>
                <p className="text-xs text-[var(--color-text-secondary)] mt-0.5">
                  Your KYC submission was rejected. Please correct the details and resubmit.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-[var(--color-text-secondary)] mt-1">Complete your domestic KYC verification</p>
              </div>
            </div>
            ...
            {submitted && kycStatus === 'pending' && (
              <div className="bg-[#d4af26]/5 border border-[#d4af26]/20 rounded-xl p-8 text-center space-y-4">
                ...
                <h3 className="text-lg font-bold text-[var(--color-text-primary)]">KYC Submitted Successfully!</h3>
                <p className="text-sm text-[var(--color-text-secondary)] max-w-md mx-auto">
                  Thank you for submitting your KYC details. Our team will carefully review your documents and verify your identity. You will be notified once the review is complete.
                </p>
            ...
            {isEditable && !submitted && (
              <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => updateForm('aadhar_number', e.target.value.replace(/[^0-9\s]/g, ''))}
                        maxLength={14}
                      />
                    </div>
                    <div>
                      <label className="text-sm text-[var(--color-text-primary)] mb-2 block font-medium">GST Number</label>
                      <input
                        type="text"
                ...
                <div className="space-y-4 pt-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div className="md:col-span-2">
                      <label className="text-sm text-[var(--color-text-primary)] mb-2 block font-medium">Business Name</label>
                      <input
                        type="text"
                ...
                <div className="space-y-4 pt-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                      <label className="text-sm text-[var(--color-text-primary)] mb-2 block font-medium">Bank Name</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="text-sm text-[var(--color-text-primary)] mb-2 block font-medium">Bank Account Number</label>
                      <input
                        type="text"
                        placeholder="Enter account number"
                        className={inputClass}
                        value={form.bank_account_number}
                        onChange={(e) => updateForm('bank_account_number', e.target.value.replace(/[^0-9]/g, ''))}
                      />
                    </div>
                    <div>
                      <label className="text-sm text-[var(--color-text-primary)] mb-2 block font-medium">IFSC Code</label>
                      <input
                        type="text"
                ...
                <div className="flex gap-3 pt-4">
                  ...
                  <button
                    onClick={() => setForm({ full_name: '', business_name: '', pan_number: '', aadhar_number: '', gst_number: '', bank_account_number: '', ifsc_code: '', bank_name: '' })}
            ...
            {(kycStatus === 'pending' || kycStatus === 'verified') && (
              <div className="space-y-4 pt-2">
                <h4 className="text-sm font-semibold text-[var(--color-text-primary)]">Submitted Details</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {[
                    ...
                    <div key={item.label} className="bg-[var(--color-border)]/30 rounded-lg px-4 py-3">
                      <p className="text-[10px] uppercase tracking-wider text-[var(--color-text-secondary)] mb-1">{item.label}</p>
        ...
        {activeTab === 'international' && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              <span className="bg-gray-500/20 text-gray-400 text-xs font-bold px-4 py-2 rounded-md">COMING SOON</span>
            </div>
            <div className="bg-[var(--color-border)]/20 rounded-xl p-10 text-center space-y-3">
              ...
              <h4 className="text-base font-semibold text-[var(--color-text-primary)]">International KYC</h4>
              <p className="text-sm text-[var(--color-text-secondary)] max-w-sm mx-auto">
                International KYC verification will be available soon. Complete your domestic KYC first to get started.
              </p>
            </div>
          </div>
        )}
