...
const labelToggles = [
  { name: 'Order value on labels', key: 'show_order_value', description: 'Shows order value in COD & Prepaid orders.' },
  { name: 'COD amount on label', key: 'show_cod_amount', description: 'Displays COD amount on the label.' },
  { name: 'Buyer mobile number', key: 'show_buyer_mobile', description: 'Shows buyer phone number on the label.' },
  { name: 'Shipper mobile numbers', key: 'show_shipper_mobile', description: 'Displays shipper phone number on the label.' },
  { name: 'Product name', key: 'show_product_name', description: 'Shows product name on the label.' },
  { name: 'Services T&C', key: 'show_services_tc', description: 'Shows services T&C on the label.' },
];
...
const LabelSettings = () => {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        const res = await api.get('/api/auth/settings/label/', { skipLoading: true });
        const data = res.data;
  ...
  const handleFileSelect = (e) => {
    ...
    const validTypes = ['image/png', 'image/webp', 'image/jpeg', 'image/jpg', 'image/gif'];
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // If logo file is selected, we'd need to upload to Cloudinary first
      // or send URL; for now the backend accepts label_logo_url
      if (logoFile) {
        // Upload to Cloudinary via backend by sending the file
        // The serializer/model stores a URL, so we handle it as a URL field
        // For simplicity, store the current URL as-is
      ...
      <div className="bg-[var(--color-bg-surface)] rounded-lg p-6 border border-[var(--color-border)] space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            {(logoPreview || labelLogoUrl) && (
              <div className="w-16 h-16 rounded-lg border border-[var(--color-border)] bg-white/5 flex items-center justify-center overflow-hidden">
                <img src={logoPreview || labelLogoUrl} alt="Logo" className="max-w-full max-h-full object-contain" />
              </div>
            )}
        ...
        <div className="space-y-0">
          {labelToggles.map((item) => (
            <div key={item.key} className="border-b border-[var(--color-border)] py-4">
              <div className="flex items-center justify-between">
                ...
                >
                  <span className={`inline-block h-5 w-5 transform rounded-full bg-white transition-transform ${toggles[item.key] ? 'translate-x-6' : 'translate-x-1'}`} />
                </button>
              </div>
