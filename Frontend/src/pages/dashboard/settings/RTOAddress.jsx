...
import { FaPen, FaTrash, FaPlus, FaTimes, FaCheckCircle, FaUndoAlt } from 'react-icons/fa';
...
const RTOAddress = () => {
  ...
  const fetchAddresses = async () => {
    try {
      const res = await api.get('/api/auth/settings/rto-address/', { skipLoading: true });
  ...
  const handleSave = async () => {
    ...
    // Build clean payload — only send email if valid or empty
    const payload = { ...form };
  ...
  const toggleStatus = async (addr) => {
    try {
      await api.patch(`/api/auth/settings/rto-address/${addr.id}/`, { is_active: !addr.is_active });
      fetchAddresses();
    } catch {
      // Silent fail
    }
      ...
      <div className="bg-[var(--color-bg-surface)] rounded-lg p-5 border border-[var(--color-border)] space-y-4">
        ...
        {addresses.length === 0 ? (
          <div className="text-center py-16 space-y-3">
            ...
            <p className="text-sm text-[var(--color-text-secondary)]">No RTO addresses added yet</p>
            <p className="text-xs text-[var(--color-text-secondary)] max-w-sm mx-auto">
              RTO (Return to Origin) addresses are used when a shipment is undeliverable and needs to be returned.
            </p>
            <button
              onClick={() => openModal()}
            ...
            <table className="w-full min-w-[700px]">
              <thead>
                <tr className="bg-[#d4af26]/10 border border-[#d4af26]/30">
                  {['ID', 'Contact', 'Pincode', 'Status', 'Primary', 'Address', 'Actions'].map((col) => (
              ...
              <tbody>
                {addresses.map((addr) => (
                  <tr key={addr.id} className="border-b border-[var(--color-border)] hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                    <td className="p-4 text-sm font-bold text-[var(--color-text-primary)]">{addr.id}</td>
                    <td className="p-4">
                    ...
                    <td className="p-4 text-sm text-[var(--color-text-primary)]">{addr.pincode}</td>
                    <td className="p-4">
                      <button
                    ...
                    <td className="p-4 text-xs text-[var(--color-text-secondary)] max-w-[200px] whitespace-pre-line">{addr.address}</td>
                    <td className="p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="flex items-center gap-4 pt-6 md:col-span-2">
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    />
                    <span className="text-sm text-[var(--color-text-primary)]">Set as Primary</span>
                  </label>
                </div>
              </div>
