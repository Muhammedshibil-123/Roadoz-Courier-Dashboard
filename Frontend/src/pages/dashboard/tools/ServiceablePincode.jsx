...
const ServiceablePincode = () => {
  ...
  const handleCheck = async () => {
    ...
    try {
      const res = await api.get(`/api/auth/check-pincode/?pincode=${pincode}`, { skipLoading: true });
      setResult(res.data);
    } catch (err) {
      setError(err.response?.data?.error || 'Failed to check pincode. Please try again.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="flex-1 p-6 space-y-5">
      ...
      <div className="bg-[var(--color-bg-surface)] rounded-lg p-6 border border-[var(--color-border)]">
        <div className="flex items-center gap-2 mb-5">
          ...
          <h2 className="text-lg font-semibold text-[var(--color-text-primary)]">Check Serviceable Pincode</h2>
        </div>
        ...
        <div className="flex flex-wrap items-end gap-4 max-w-xl">
          <div className="flex-1 min-w-[200px]">
            <label className="text-sm text-[var(--color-text-primary)] mb-2 block font-medium">Pincode</label>
            <input
              type="text"
        ...
        {/* Result */}
        {result && (
          <div className={`mt-5 rounded-lg px-5 py-5 border ${
            result.serviceable
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                <h3 className={`text-lg font-bold ${result.serviceable ? 'text-green-400' : 'text-red-400'}`}>
                  {result.serviceable ? 'Serviceable ✓' : 'Not Serviceable ✗'}
                </h3>
                <p className="text-sm text-[var(--color-text-secondary)] mt-1">
                  Pincode: <span className="font-bold text-[var(--color-text-primary)]">{result.pincode}</span>
                </p>
                <p className="text-sm text-[var(--color-text-secondary)] mt-1">
